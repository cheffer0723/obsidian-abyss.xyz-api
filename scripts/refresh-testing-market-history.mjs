/**
 * Refresh local harness market history from Yahoo daily adj-close.
 * Research/historical testing only — not a live signal feed.
 *
 * Usage: npm run refresh:harness-data
 * Then publish data/testing-harness/market-history.json to the site
 * public/harness/market-history.json path for production.
 */
...
for (const symbol of symbols) {
  ...
  fs.writeFileSync(
    path.join(outDir, `${symbol}.csv`),
    ["Date,Adj Close", ...bars.map(([date, close]) => `${date},${close}`)].join("\n") + "\n",
  );
