...
router.get("/auth/verify", async (req, res, next) => { try { const token = z.string().min(20).parse(req.query.token); const returnTo = returnToSchema.catch("/").parse(req.query.returnTo); const ok = await verifyMagicLink(token, res); const appUrl = (process.env.AUTH_APP_URL || "https://obsidian-abyss.xyz").replace(/\/$/, ""); const separator = returnTo.includes("?") ? "&" : "?"; res.redirect(`${appUrl}${returnTo}${separator}auth=${ok ? "success" : "invalid"}`); } catch (e) { next(e); } });
